...
from ...models import db, RemoteServerConfig, Beehive, Alert
...
def push_to_remote(config_id):
    ...
    # Naive UTC for DB comparisons (SQLAlchemy DateTime columns are tz-unaware)
    ...
    for alert in Alert.query.filter(Alert.created_at >= since_naive).all():
